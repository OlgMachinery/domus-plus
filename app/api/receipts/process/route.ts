  ...
  try {
    ...
    try {
      if (admin) {
        ...
        await admin.from('receipts').update({ assigned_transaction_id: createdTransaction.id, status: 'assigned' }).eq('id', receiptData.id)
        ...
        if (assignedUserId === authUser.id) {
          ...
          if (!txError && txData?.id) {
            createdTransaction = txData
            await supabase.from('receipts').update({ assigned_transaction_id: createdTransaction.id, status: 'assigned' }).eq('id', receiptData.id)
          }
        }
      }
    } catch (err: any) {
